...
O configurador altera apenas `tui.status_line` em `~/.codex/config.toml` e habilita no rodapé nativo: modelo, contexto, tokens usados, branch e diretório; limites de 5h e semanal aparecem quando forem fornecidos pela conta. Ao concluir, informe que é necessário reiniciar o Codex para aplicar o rodapé. No início da sessão, o hook mostra na TUI o último estado salvo; depois de cada turno, atualiza a cota efetiva, o tempo até o reset e o percentual do contexto. O relatório detalhado também pode ser consultado com o script `usage.js` dentro do cache do plugin ou diretamente de um checkout do repositório.
