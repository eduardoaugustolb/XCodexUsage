...
O comando de configuração modifica apenas `tui.status_line` em `~/.codex/config.toml`. Você também pode escolher/reordenar os itens interativamente com `/statusline` dentro do Codex. Ao iniciar, o plugin mostra na TUI o último estado salvo; ao fim de cada turno, atualiza uma mensagem destacada com a cota efetiva da conta, o tempo até o reset e o percentual do contexto da sessão — inclusive para planos cuja janela não seja exibida pelo rodapé nativo.
...
- A TUI identifica essas mensagens como saídas de hook, portanto o rótulo `SessionStart` ou `Stop` aparece antes do painel. Uma quebra de linha separa o rótulo da borda superior.
